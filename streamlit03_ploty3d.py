...
fig1 = px.line(vendas_por_dia, x="dateCreated", y="totalValue", markers=True, labels={"dateCreated": "Data", "totalValue": "Total (R$)"}, title="📈 Evolução das Vendas")
fig1.update_traces(text=vendas_por_dia["totalValueFormatado"], mode="lines+markers+text", textposition="top center")
st.plotly_chart(fig1, use_container_width=True)

# 📊 Gráfico: Itens mais vendidos
mais_vendidos = df_filtrado["item_title"].value_counts().head(10).reset_index()
mais_vendidos.columns = ["item_title", "quantidade"]
fig2 = px.bar(mais_vendidos, x="quantidade", y="item_title", orientation="h", labels={"item_title": "Produto", "quantidade": "Quantidade"}, title="📊 Top 10 Produtos por Quantidade Vendida", text="quantidade")
fig2.update_traces(textposition="outside")
st.plotly_chart(fig2, use_container_width=True)

# 📉 Gráfico: Lucro por Produto
...
fig3 = px.bar(lucro, x="lucro_unitario", y="item_title", orientation="h", text="lucro_formatado", labels={"item_title": "Produto", "lucro_unitario": "Lucro Total (R$)"}, title="📉 Top 10 Produtos por Lucro Total")
fig3.update_traces(textposition="outside")
st.plotly_chart(fig3, use_container_width=True)

# Adicionar aqui os novos gráficos... (continuação do código)
